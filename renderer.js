...
function getHexesToRender() {
	...
	if (gameState.gamePhase === 'initial') {
		...
		// Add all hexes within render distance of each tile
		for (const key of gameState.tiles.keys()) {
			const [q, r] = key.split(',').map(Number);
			
			for (let dq = -RENDER_HORIZON; dq <= RENDER_HORIZON; dq++) {
				for (let dr = -RENDER_HORIZON; dr <= RENDER_HORIZON; dr++) {
					// Axial distance formula: (|dq| + |dr| + |dq+dr|) / 2
					const dist = (Math.abs(dq) + Math.abs(dr) + Math.abs(dq + dr)) / 2;
					if (dist <= RENDER_HORIZON) {
						hexes.add(getHexKey(q + dq, r + dr));
					}
				}
			}
		}
